...
class SaleOrder(models.Model):
    ...
    def _get_order_count(self):
        for order in self:
            ...
            order.update({
                'purchase_orders_count': len(purchases),
                'manufacture_order_count': len(manufactures),
            })

    purchase_orders_count = fields.Integer(string='# of purchases', compute='_get_order_count', readonly=True)
    manufacture_order_count = fields.Integer(string='# of manufactures', compute='_get_order_count', readonly=True)
    project_name = fields.Char('Project Name')
    po_created = fields.Boolean(string='PO Created', copy=False)
    mo_created = fields.Boolean(string='MO Created', copy=False)
    x_customer_po = fields.Char(string='Customer PO')
    ...
    def create_po_procurement_order(self, product_id, qty, from_mo=False):
        for vendor in product_id.seller_ids:
            ...
            if rfq:
                ...
                #     if line.product_id.id == line.product_id.id and line.sale_production_id.id == self.id:
                #         line_exist = True
                # if not line_exist:
    ...
    def create_mo_procurement_order(self, product_id, qty, from_mo=False):
        ...
        product_tmpl_bom_id = self.env['mrp.bom'].sudo().search(
            [('product_tmpl_id', '=', product_id.product_tmpl_id.id)], order="id desc", limit=1)
        if product_bom_id:
            prod_id = product_bom_id
        elif product_tmpl_bom_id:
            prod_id = product_tmpl_bom_id
        ...
        if prod_id:
            ...
            if mrp:
                ...
                mrp.action_confirm()
            sale_log_vals = {'sale_order_id': self.id, 'status': 'sale', 'ordered_qty': qty, 'avail_qty': virtual_qty,
    ...
    def action_done(self):
        for order in self:
            for line in self.env['production.sale.order'].sudo().search(
    ...
    def action_cancel(self):
        for record in self:
            ...
            mos = self.env['mrp.production'].sudo().search(
                [('sale_production_id', '=', record.id), ('state', '!=', 'cancel')])
            for prod in self.env['production.sale.order'].sudo().search(
            ...
            for mo in mos:
                total_qty = 0
                if mo.state not in ['draft', 'confirmed', 'cancel']:
            ...
            record.po_created = False
            record.mo_created = False
